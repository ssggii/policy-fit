 ...
 * RequestToAnswersMapper → PolicyEvaluator(정적 out_of_scope + base rule + 범위 게이트)까지, HTTP
 * 계층을 제외한 판정 파이프라인 전체를 정책별·케이스별로 검증한다(HTTP/JSON 직렬화 자체는
 * VerdictsControllerTest가 별도로 담당).
 *
 * {@link #POLICY_IDS}에 등록된 모든 정책의 골든셋을 이 하나의 테스트 클래스로 커버한다 — 정책이
 * 늘어도 별도 테스트 클래스를 만들지 않고 이 목록만 확장한다. 게이트 없는 정책(jutaek-dream·
 * beotimmok-jeonse)과 범위 게이트 정책(cheongnyeon-wolse)을 PolicyEvaluator가 동일 경로로 처리한다.
 */
class GoldenSetTest {

    /** 골든셋을 갖춘 정책 ID 목록 — 정책 추가마다 이 목록만 확장하면 된다. */
    private static final List<String> POLICY_IDS = List.of("jutaek-dream", "beotimmok-jeonse", "cheongnyeon-wolse");
    ...
    void goldenCase(PolicyGoldenCase policyCase) {
        ...
        PolicyEvaluation evaluation = POLICY_EVALUATOR.evaluate(policy.rule(), policy.outOfScopeGate(), answers);
